...
class CorpusStore:
    ...
    def replace_kg_entities(self, run_id: str, entities: list[dict[str, Any]]) -> None:
        self.execute("DELETE FROM kg_entities WHERE run_id=?", (run_id,))
        self.execute("DELETE FROM kg_entity_aliases WHERE run_id=?", (run_id,))
        now = _now()
    ...
    def replace_kg_relations(self, run_id: str, relations: list[dict[str, Any]]) -> None:
        self.execute("DELETE FROM kg_relations WHERE run_id=?", (run_id,))
        now = _now()
    ...
    def replace_kg_embeddings(self, run_id: str, embeddings: list[dict[str, Any]], target_type: str | None = None) -> None:
        if target_type:
            self.execute("DELETE FROM kg_embeddings WHERE run_id=? AND target_type=?", (run_id, target_type))
        else:
            self.execute("DELETE FROM kg_embeddings WHERE run_id=?", (run_id,))
        now = _now()
