...
from backend.services.pdf_ocr import (
    PDFOCRResult,
    needs_deepseek_ocr_assist,
    ocr_pdf_with_easyocr,
...
def worker_command(args: argparse.Namespace) -> int:
    ...
    if not args.once and args.run_id and not _has_active_jobs(store, args.run_id):
...
def _next_job(store: CorpusStore, run_id: str | None) -> dict[str, Any] | None:
    return store.claim_next_job(run_id)
...
def process_extract_job(store: CorpusStore, job: dict[str, Any], ocr: Mode) -> None:
    ...
    should_ocr = ocr == "always" and source_type in {"pdf", "image"}
    should_ocr = should_ocr or (bool(doc.metadata.get("ocr_required")) and ocr != "off")
    if should_ocr:
...
def _run_pdf_ocr(path: Path) -> PDFOCRResult:
    """Run PDF OCR with the configured engine.

    When HF_OCR_ENGINE=easyocr we use GPU EasyOCR; if EasyOCR/torch is missing
    or fails to init (skipped result), we transparently fall back to the
    CPU Tesseract path so ingestion never breaks.
    """

    if settings.ocr_engine == "easyocr":
        result = ocr_pdf_with_easyocr(
            path,
...
def _has_active_jobs(store: CorpusStore, run_id: str) -> bool:
    row = store.fetchone(
        "SELECT COUNT(*) AS count FROM ingest_jobs WHERE run_id=? AND status IN ('pending', 'running')",
        (run_id,),
    )
