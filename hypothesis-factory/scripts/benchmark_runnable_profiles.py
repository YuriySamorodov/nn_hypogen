...
def main() -> None:
    ...
    for profile in pipeline_profiles():
        profile_input = pipeline_input.model_copy(update={"retrieval_mode": profile.retrieval_mode})
        if profile.retrieval_mode not in retrievers:
            retrievers[profile.retrieval_mode] = build_retriever(kb, profile_input)
        retriever = retrievers[profile.retrieval_mode]
        hypotheses, metrics = run_pipeline_profile(profile, kb, profile_input, retriever)
        rows.append(metrics)
        examples[profile.id] = [
            {
                "rank": idx,
...
def _markdown(payload: dict) -> str:
    ...
    for idx, row in enumerate(payload["profiles"], 1):
        lines.append(
            f"| {idx} | {row['profile_id']} | {row['profile_name']} | {row.get('retrieval_mode', 'tfidf')} | "
